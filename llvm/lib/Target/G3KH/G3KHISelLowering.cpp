//===-- G3KHISelLowering.cpp - G3KH DAG Lowering Implementation  ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the G3KHTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "g3kh-lower"

static cl::opt<bool>G3KHNoLegalImmediate(
  "g3kh-no-legal-immediate", cl::Hidden,
  cl::desc("Enable non legal immediates (for testing purposes only)"),
  cl::init(false));

G3KHTargetLowering::G3KHTargetLowering(const TargetMachine &TM,
                                           const G3KHSubtarget &STI)
    : TargetLowering(TM) {
  ...
  setStackPointerRegisterToSaveRestore(G3KH::SP);
  ...
  if (STI.hasHWMult16()) {
    ...
    setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::G3KH_BUILTIN);
  }

  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::OLE_F64, CallingConv::G3KH_BUILTIN);
  setLibcallCallingConv(RTLIB::OGT_F64, CallingConv::G3KH_BUILTIN);
...
SDValue G3KHTargetLowering::LowerOperation(SDValue Op,
...
bool G3KHTargetLowering::shouldAvoidTransformToShift(EVT VT,
                                                       unsigned Amount) const {
  return !(Amount == 8 || Amount == 9 || Amount<=2);
}

// Implemented to verify test case assertions in
// tests/codegen/g3kh/shift-amount-threshold-b.ll
bool G3KHTargetLowering::isLegalICmpImmediate(int64_t Immed) const {
  if (G3KHNoLegalImmediate)
    return Immed >= -32 && Immed < 32;
  return TargetLowering::isLegalICmpImmediate(Immed);
}

//===----------------------------------------------------------------------===//
//                       G3KH Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
G3KHTargetLowering::getConstraintType(StringRef Constraint) const {
...
G3KHTargetLowering::getRegForInlineAsmConstraint(
...
#include "G3KHGenCallingConv.inc"
...
static void AnalyzeVarArgs(CCState &State,
  ...
  State.AnalyzeCallOperands(Outs, CC_G3KH_AssignStack);
}

static void AnalyzeVarArgs(CCState &State,
                           const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeFormalArguments(Ins, CC_G3KH_AssignStack);
}
...
static void AnalyzeArguments(CCState &State,
  ...
  bool Builtin = (State.getCallingConv() == CallingConv::G3KH_BUILTIN);
  if (Builtin) {
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UsedStack && Parts == 2 && RegsLeft == 1) {
      ...
      UsedStack = true;
      CC_G3KH_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_G3KH);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_G3KH);
}
...
SDValue G3KHTargetLowering::LowerFormalArguments(
...
SDValue
G3KHTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // G3KH target does not yet support tail call optimization.
  isTailCall = false;

  switch (CallConv) {
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::G3KH_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue G3KHTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  G3KHMachineFunctionInfo *FuncInfo = MF.getInfo<G3KHMachineFunctionInfo>();
...
bool
G3KHTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  return CCInfo.CheckReturn(Outs, RetCC_G3KH);
}

SDValue
G3KHTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool isVarArg,
                                  const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  if (CallConv == CallingConv::G3KH_INTR && !Outs.empty())
  ...
  unsigned Opc = (CallConv == CallingConv::G3KH_INTR ?
                  G3KHISD::RETI_FLAG : G3KHISD::RET_FLAG);
...
SDValue G3KHTargetLowering::LowerCCCCallTo(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, dl, G3KH::SP, PtrVT);
  ...
  Chain = DAG.getNode(G3KHISD::CALL, dl, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
///
SDValue G3KHTargetLowering::LowerCallResult(
...
SDValue G3KHTargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(G3KHISD::RRCL, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? G3KHISD::RLA : G3KHISD::RRA),
                         dl, VT, Victim);

  return Victim;
}

SDValue G3KHTargetLowering::LowerGlobalAddress(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  return DAG.getNode(G3KHISD::Wrapper, SDLoc(Op), PtrVT, Result);
}

SDValue G3KHTargetLowering::LowerExternalSymbol(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  return DAG.getNode(G3KHISD::Wrapper, dl, PtrVT, Result);
}

SDValue G3KHTargetLowering::LowerBlockAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc dl(Op);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  G3KHCC::CondCodes TCC = G3KHCC::COND_INVALID;
  switch (CC) {
  default: llvm_unreachable("Invalid integer condition!");
  case ISD::SETEQ:
    TCC = G3KHCC::COND_E;     // aka COND_Z
    ...
    break;
  case ISD::SETNE:
    TCC = G3KHCC::COND_NE;    // aka COND_NZ
    // Minor optimization: if LHS is a constant, swap operands, then the
    // constant can be folded into comparison.
    if (LHS.getOpcode() == ISD::Constant)
      std::swap(LHS, RHS);
  ...
  case ISD::SETULT:
    ...
    TCC = G3KHCC::COND_LO;    // aka COND_NC
    break;
  case ISD::SETLE:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  ...
  return DAG.getNode(G3KHISD::CMP, dl, MVT::Glue, LHS, RHS);
}


SDValue G3KHTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(G3KHISD::BR_CC, dl, Op.getValueType(),
                     Chain, Dest, TargetCC, Flag);
}

SDValue G3KHTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    ...
    return DAG.getNode(G3KHISD::SELECT_CC, dl, Op.getValueType(), Ops);
  }
}

SDValue G3KHTargetLowering::LowerSELECT_CC(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDValue LHS    = Op.getOperand(0);
  SDValue RHS    = Op.getOperand(1);
...
SDValue G3KHTargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue
G3KHTargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  G3KHMachineFunctionInfo *FuncInfo = MF.getInfo<G3KHMachineFunctionInfo>();
...
SDValue G3KHTargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue G3KHTargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue G3KHTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  G3KHMachineFunctionInfo *FuncInfo = MF.getInfo<G3KHMachineFunctionInfo>();
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue G3KHTargetLowering::LowerJumpTable(SDValue Op,
                                             SelectionDAG &DAG) const {
    JumpTableSDNode *JT = cast<JumpTableSDNode>(Op);
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Result = DAG.getTargetJumpTable(JT->getIndex(), PtrVT);
    return DAG.getNode(G3KHISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool G3KHTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *G3KHTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((G3KHISD::NodeType)Opcode) {
  case G3KHISD::FIRST_NUMBER:       break;
  case G3KHISD::RET_FLAG:           return "G3KHISD::RET_FLAG";
  case G3KHISD::RETI_FLAG:          return "G3KHISD::RETI_FLAG";
  case G3KHISD::RRA:                return "G3KHISD::RRA";
  case G3KHISD::RLA:                return "G3KHISD::RLA";
  case G3KHISD::RRC:                return "G3KHISD::RRC";
  case G3KHISD::RRCL:               return "G3KHISD::RRCL";
  case G3KHISD::CALL:               return "G3KHISD::CALL";
  case G3KHISD::Wrapper:            return "G3KHISD::Wrapper";
  case G3KHISD::BR_CC:              return "G3KHISD::BR_CC";
  case G3KHISD::CMP:                return "G3KHISD::CMP";
  case G3KHISD::SETCC:              return "G3KHISD::SETCC";
  case G3KHISD::SELECT_CC:          return "G3KHISD::SELECT_CC";
  case G3KHISD::DADD:               return "G3KHISD::DADD";
  }
  return nullptr;
}

bool G3KHTargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool G3KHTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool G3KHTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // G3KH implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool G3KHTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // G3KH implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && VT1 == MVT::i8 && VT2 == MVT::i16;
}

bool G3KHTargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
MachineBasicBlock *
G3KHTargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&G3KH::GR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&G3KH::GR8RegClass);
